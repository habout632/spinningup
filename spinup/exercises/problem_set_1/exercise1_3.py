...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main'):
        ...
        #                     #
        #######################
        # pi, q1, q2, q1_pi = actor_critic(x=x_ph, a=a_ph)
        pi, q1, q2, q1_pi = actor_critic(x=x_ph, a=a_ph, **ac_kwargs)
    ...
    with tf.variable_scope('target'):
        ...
        #                     #
        #######################
        pi_targ, _, _, _ = actor_critic(x=x2_ph, a=a_ph, **ac_kwargs)
    ...
    with tf.variable_scope('target', reuse=True):
        ...
        #                     #
        #######################
        # epsilon = np.random.normal(loc=0.0, scale=target_noise)
        epsilon = tf.distributions.Normal(loc=0.0, scale=target_noise).sample(sample_shape=act_dim)
        smoothed_action = tf.clip_by_value(pi_targ + tf.clip_by_value(epsilon, -noise_clip, noise_clip), -act_limit, act_limit)
        ...
        #                     #
        #######################
        # _, q1_target, q2_target, _ = actor_critic(x=x_ph, a=smoothed_action)
        _, q1_target, q2_target, _ = actor_critic(x=x2_ph, a=smoothed_action, **ac_kwargs)
    ...
    #                     #
    #######################
    y_target = r_ph + gamma * (1 - d_ph) * tf.minimum(q1_target, q2_target)
    ...
    #                     #
    #######################
    # crucially import minus
    pi_loss = -tf.reduce_mean(q1_pi)
    q1_loss = tf.reduce_mean(tf.squared_difference(q1, y_target))
    q2_loss = tf.reduce_mean(tf.squared_difference(q2, y_target))
