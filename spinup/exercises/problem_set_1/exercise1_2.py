...
def mlp(x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
    ...
    #######################
    inputs = x
    for dim in hidden_sizes[:-1]:
        inputs = tf.layers.dense(inputs=inputs, units=dim, activation=activation)
    outputs = tf.layers.dense(inputs=inputs, units=hidden_sizes[-1], activation=output_activation)
    return outputs
...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    mu = mlp(x, hidden_sizes=hidden_sizes + shape, activation=activation, output_activation=output_activation)

    action_shape = a.get_shape().as_list()
    # action_shape = tuple(a.get_shape.as_list())
    log_std_tensor = tf.convert_to_tensor(action_shape[1] * -0.5, np.float32)
    # tf.convert_to_tensor([-0.5] * action_shape, dtype=tf.float32)
    log_std = tf.Variable(log_std_tensor, name="log_std")

    pi = tf.distributions.Normal(loc=mu, scale=tf.exp(log_std)).sample(sample_shape=shape)
...
if __name__ == '__main__':
    ...
    ppo(env_fn=lambda: gym.make('InvertedPendulum-v2'),
        ac_kwargs=dict(policy=mlp_gaussian_policy, hidden_sizes=(64,), activation=tf.tanh,
